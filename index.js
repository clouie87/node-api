...
voteRouter.get('/', function(req, res) {
  ...
  postgres.client.query(sql, function (err, result) {
    ...
    var sql = 'SELECT COUNT(votes.*) as votecount GROUP BY v_id, votes.*, photo.c_id as c_id FROM votes, photo WHERE votes.p_id = photo.id AND votes.u_id = $3 OFFSET $1 LIMIT $2';
    postgres.client.query(sql, [offset, limit, req.user.id], function (err, result) {
      if (err) {
        console.error(err);
        res.statusCode = 500;
